...
class FacebookAdsInsightsToGCSOperator(BaseOperator):
    ...
    :param google_cloud_storage_conn_id:              The destination GCS connection id.
    :type google_cloud_storage_conn_id:               string
    ...
    def execute(self, context):
        facebook_conn = FacebookAdsHook(self.facebook_conn_id, self.facebook_access_token)
        gcs_conn = GoogleCloudStorageHook(self.google_cloud_storage_conn_id)

        time_range = {
            'since': datetime.strptime(self.since, '%Y-%m-%dT%H:%M:%S+00:00').strftime('%Y-%m-%d'),
            'until': datetime.strptime(self.until, '%Y-%m-%dT%H:%M:%S+00:00').strftime('%Y-%m-%d')
        }


        file_name = '/tmp/{key}.jsonl'.format(key=self.gcs_key)
        object_name = '{key}.json'.format(key=self.gcs_key)
        print('file_name:')
        print(file_name)
        with open(file_name, 'w') as insight_file:
            insights = facebook_conn.get_insights_for_account_id(self.account_ids, 
                                                                    self.insight_fields, 
                                                                    self.breakdowns, 
                                                                    time_range, 
                                                                    self.time_increment, 
        ...
        gcs_conn.upload(self.gcs_bucket, object=object_name, filename=file_name)
        os.remove(file_name)
